...
describe ActiveRecord do
  context 'when finding' do
    before do
      allow(Customer).to receive(:find).and_raise("Unexpected call to 'find'")
      allow(Customer).to receive(:expect).and_raise("Unexpected call to 'expect'")
    end

    context 'when using .where:' do
      before do
        allow(Customer).to receive(:where).and_call_original
      end
    ...
    context 'without needing .where' do
